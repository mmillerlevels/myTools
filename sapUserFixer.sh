...
do
	...
	echo "$USERID"
	echo "Making Updates ~ "
	psql -U reachengine -d studio -c "UPDATE security_user SET user_name = '$portal_id' WHERE id = $USERID" -tA
	psql -U reachengine -d studio -c "UPDATE security_user SET email_address = '$sap_email' WHERE id = $USERID" -tA
